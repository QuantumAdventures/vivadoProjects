...
entity PID is
    generic  (input_size   : positive;
	          control_size : positive);
	port     (clk_i      : in  std_logic;
			  enable     : in  std_logic;
			  clkEnable  : in  std_logic;
			  input_i    : in  std_logic_vector(input_size-1 downto 0);
			  set_point_RAM  : in   std_logic_vector(input_size-1 downto 0);
			  set_point_ADC  : in   std_logic_vector(input_size-1 downto 0);
			  ...
			  high_bound : std_logic_vector(input_size-1  downto 0);
	          low_bound  : std_logic_vector(input_size-1  downto 0);
...
signal output_sf : sfixed(0 downto -(control_size-1)) := (others => '0');

-- signals for clamping
signal signComp      : std_logic := '0';
signal clamping      : std_logic := '0';
signal high_bound_sf : sfixed(0 downto -(input_size-1 )) := (others => '0');
signal low_bound_sf : sfixed(0 downto -(input_size-1 )) := (others => '0');
            ...
            case PS is
			    ...
			    when ST0 =>
			        if (clkEnable = '1') then
			            ...
			            if (enable = '0') then
			                control <= (others => '0');
			                integral_sf <= (others => '0');
			                
			            else
			             
                            case set_point_SEL is
                                when '0' => set_point_sf <= to_sfixed(set_point_RAM,set_point_sf);
                                when '1' => set_point_sf <= to_sfixed(set_point_ADC,set_point_sf);
                                when others => null;
                            end case;
				...
				when ST5 =>
					...
					control  <= std_logic_vector(unsigned(to_slv(output_sf)));
                    NS <= ST0;
